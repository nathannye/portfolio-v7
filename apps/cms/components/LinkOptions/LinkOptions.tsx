...
export default function LinkOptions(props) {
	...
	const internalMembers = members.filter((member: any) => member.name === 'page')
	const externalMembers = members.filter((member: any) => member.name === 'url')
	const advancedMembers = members.filter(
		(member: any) => member.name === 'advanced',
	)
	...
	return (
		<Stack className={styles} space={3}>
			...
			<Flex direction="column" gap={4}>
				...
				{/* Always show advanced fields regardless of link type */}
				{advancedMembers.map((member: any) => (
					<MemberField
						renderInput={renderInput}
						renderField={renderField}
						renderItem={renderItem}
